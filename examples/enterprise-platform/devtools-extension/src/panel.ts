...
const state = {
  ...
  sharedModules: {
    registered: {} as Record<string, Array<Record<string, unknown>>>,
    loaded: {} as Record<string, Record<string, unknown>>,
  },
  importMap: null as {
...
function jsonTree(value: unknown, indent = 0, isLast = true): string {
  ...
  if (value === null || value === undefined)
    return `<span class="snull">null</span>${isLast ? '' : ','}`;
  if (typeof value === 'string')
    return `<span class="ss">"${esc(value)}"</span>${isLast ? '' : ','}`;
  ...
  if (Array.isArray(value)) {
    ...
    const items = value
      .map((v, i) => `${pad}  ${jsonTree(v, indent + 1, i === value.length - 1)}`)
      .join('\n');
  ...
  tab.addEventListener('click', () => {
    ...
    panels.forEach((p) => {
      ...
      p.style.display = isTarget
        ? ['events', 'flow'].includes(activeTab.value)
          ? 'flex'
          : 'block'
        : 'none';
    });
...
const legendItems = [
  { label: 'Mounted', color: '#3fb950' },
  { label: 'Frozen', color: '#d29922' },
  { label: 'Not Loaded', color: '#484f58' },
  { label: 'Loading', color: '#58a6ff' },
...
function renderTopology(): void {
  ...
  for (const edge of PARCEL_EDGES) {
    ...
    pt.textContent = 'Parcel';
    topoSvg.appendChild(pt);
  }

  // Prefetch probability edges
  for (const s of state.prefetchStats) {
    const fp = positions[s.from];
    const tp = positions[s.to];
    if (!fp || !tp || s.ratio < 0.1) continue;
    const a = angleBetween(fp.x, fp.y, tp.x, tp.y);
    ...
    const line = svg('line', {
      ...
      y2: ey,
      stroke: '#1f6feb',
      'stroke-width': String(1 + s.ratio * 2),
    ...
    prob.textContent = `${Math.round(s.ratio * 100)}%`;
  ...
  for (const appDef of APP_TOPOLOGY) {
    ...
    sl.textContent = status.toLowerCase();
    ...
    g.addEventListener('mouseenter', (e) => {
      ...
      const perfHtml = perfInfo
        ? Object.entries(perfInfo.phases)
            .map(([p, d]) => `<div>${p}: ${d.toFixed(0)}ms</div>`)
            .join('') + `<div style="font-weight:600">Total: ${perfInfo.total.toFixed(0)}ms</div>`
...
function addEvent(entry: EventEntry): void {
  ...
  if (
    ...
    (state.activeFilter === 'all' || state.activeFilter === entry.category)
  ) {
    const shouldScroll =
...
function renderState(): void {
  ...
  for (const entry of [...state.stateHistory].reverse()) {
    ...
    div.innerHTML =
      `<div class="diff-time">${fmtTime(entry.timestamp)}</div>` +
      entry.diff
        .map(
          (d) =>
...
function renderPerf(): void {
  ...
  scale.style.cssText =
    'display:flex;justify-content:space-between;color:#484f58;font-size:10px;margin-bottom:8px;padding-left:80px';
  const ticks = [
    0,
  ...
  for (const appDef of APP_TOPOLOGY) {
    ...
    barContainer.style.cssText =
      'flex:1;display:flex;height:20px;background:#161b22;border-radius:4px;overflow:hidden';
  ...
  legend.style.cssText =
    'display:flex;gap:12px;margin-top:12px;padding-top:8px;border-top:1px solid #21262d';
...
function renderDeps(): void {
  ...
  container.style.cssText =
    'padding:16px;font-family:"SF Mono",monospace;font-size:12px;overflow-y:auto;height:100%';
  ...
  for (const moduleName of moduleNames) {
    ...
    card.style.cssText =
      'background:#161b22;border:1px solid #21262d;border-radius:8px;padding:12px 16px;margin-bottom:10px';
    ...
    if (isSingleton) {
      ...
      sb.style.cssText =
        'font-size:10px;padding:1px 8px;border-radius:10px;background:#bc8cff20;color:#bc8cff;border:1px solid #bc8cff40;font-weight:500';
    ...
    if (isEager) {
      ...
      eb.style.cssText =
        'font-size:10px;padding:1px 8px;border-radius:10px;background:#58a6ff20;color:#58a6ff;border:1px solid #58a6ff40;font-weight:500';
    ...
    for (const candidate of candidates) {
      ...
      const isActive =
        loadedInfo &&
        loadedInfo['version'] === candidate['version'] &&
        loadedInfo['from'] === candidate['from'];
  ...
  if (state.importMap) {
    ...
    headerRow.innerHTML =
      '<span style="width:200px;flex-shrink:0">Specifier</span><span style="flex:1">URL</span>';
  ...
  switch (type) {
    ...
    case 'ESMAP_STATUS_CHANGE': {
      ...
      state.apps.set(appName, {
        name: appName,
        status: to,
        container: state.apps.get(appName)?.container ?? '',
      });
      addEvent({
        timestamp: ts,
        category: 'lifecycle',
      ...
      if (phaseInfo) {
        ...
        if (phaseInfo.endPhase && phaseStartTimes[appName][phaseInfo.endPhase]) {
          ...
          derivedPerfData[appName].total = Object.values(derivedPerfData[appName].phases).reduce(
            (sum, d) => sum + d,
            0,
          );
    ...
    case 'ESMAP_STATE_CHANGE': {
      ...
      if (diffs.length > 0) {
        ...
        addEvent({
          ...
          category: 'state',
          name: 'state:change',
          detail: diffs.map((d) => `${d.key}: ${JSON.stringify(d.newValue)}`).join(', '),
        });
        if (activeTab.value === 'state') renderState();
      }
      break;
    ...
    case 'ESMAP_ROUTE_CHANGE': {
      ...
      const targetApp = APP_TOPOLOGY.find((a) => {
        const path =
          a.name === '@enterprise/dashboard' ? '/dashboard' : `/${a.short.toLowerCase()}`;
    ...
    case 'ESMAP_PERF': {
      ...
      addEvent({
        ...
        category: 'lifecycle',
        name: `perf:${phase}`,
        detail: `${short(appName)} ${duration.toFixed(0)}ms`,
        appName,
      });
      if (activeTab.value === 'topology') renderTopology();
      if (activeTab.value === 'perf') renderPerf();
