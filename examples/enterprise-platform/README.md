...
| 기능                       | 위치                          | 설명                                                          |
| -------------------------- | ----------------------------- | ------------------------------------------------------------- |
| **공유 의존성 버전 협상**  | `shared-deps` + shell config  | `esmapSharedDeps`로 빌드 → `SharedModuleRegistry` 런타임 해석 |
| **Import Map 서버 + SSE**  | `platform-server` + shell SSE | 동적 배포/롤백, SSE로 실시간 import map 갱신                  |
| **ReadyGate 인증 게이팅**  | `auth` + shell                | 인증 전 보호 라우트 차단, `markReady` 후 해제                 |
...
| **커스텀 플러그인 작성**   | `shell/plugins/audit-log`     | EsmapPlugin 인터페이스 직접 구현, hooks/registry/router 접근  |
| **서버 fetch + fallback**  | `shell boot.ts`               | 서버에서 import map 동적 로드, 실패 시 로컬 fallback          |
...
pnpm dev      # http://localhost:3100 (shell)
```
...
shell의 SSE 클라이언트가 변경을 실시간 수신하여 상태 패널에 표시한다.
...
- `pnpm preview`는 빌드 → 시드 → 서버 + shell을 순차/동시 실행한다.
