...
```ts
function createEventBus<E extends EventMap = EventMap>(
...
```ts
function createGlobalState<T extends Record<string, unknown>>(
  initial: T,
): GlobalState<T>
```
...
```ts
function createAppProps<T extends Record<string, unknown>>(
  initial: T,
): AppProps<T>
```
