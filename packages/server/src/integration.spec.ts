 ...
 * Validates the full flow of deploy -> read -> rollback.
 */
describe('server API integration tests', () => {
  ...
  it('full flow: deploy -> read -> update -> delete', async () => {
    const { app } = createApp();

    // 1. Initial state: empty import map
    ...
    // 3. Read -- import map contains the deployed app
    ...
    // 4. Update app (deploy new version)
    ...
    // 6. Read after deletion -- app should be gone
  ...
  it('scenario: deploying multiple apps sequentially', async () => {
    const { app } = createApp();

    // Deploy 3 apps sequentially
    const apps = [
  ...
  it('deploy -> rollback workflow', async () => {
    const { app } = createApp();
